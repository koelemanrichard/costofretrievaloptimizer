...
export const LinkingAuditModal: React.FC<LinkingAuditModalProps> = ({
  ...
  // Custom header with score badge
  const customHeader = (
    <div className="flex items-center gap-4 flex-1">
      ...
      {result && (
        ...
        }`} role="status" aria-label={`Audit score: ${result.overallScore} out of 100`}>
  ...
  return (
    <Modal
      ...
      description="Analyze your internal linking structure across 4 passes"
      maxWidth="max-w-4xl"
    ...
    >
      ...
      {/* Stats bar */}
      {stats && (
        <div className="py-3 -mx-6 px-6 bg-gray-900/50 border-b border-gray-700 flex items-center gap-6 text-sm flex-shrink-0 mb-4" role="status" aria-label="Audit statistics">
      ...
      {/* Tabs */}
      <div className="py-3 -mx-6 px-6 border-b border-gray-700 flex items-center gap-2 flex-shrink-0 overflow-x-auto mb-4" role="tablist" aria-label="Audit passes">
        {PASS_TABS.map(tab => {
          ...
          return (
            ...
            >
              <span className={`font-bold ${statusColor}`} aria-hidden="true">{statusIcon}</span>
              <span>{tab.label}</span>
              {issueCount > 0 && (
                <span className="px-2 py-0.5 text-xs rounded-full bg-gray-700 text-gray-300" aria-label={`${issueCount} issues`}>
                  {issueCount}
                </span>
              )}
            </button>
      ...
      <div>
        ...
        {activeTab === 'site_overview' && siteWideResult && (
          <div
            role="tabpanel"
            id={`${tabId}-site_overview-panel`}
            aria-labelledby={`${tabId}-site_overview-tab`}
          >
            <SiteOverviewContent
    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden">
        <button
          ...
          aria-controls={`${sectionId}-links-content`}
        >
          <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-gray-500" aria-hidden="true">{expandedSection === 'links' ? '▲' : '▼'}</span>
          </div>
        </button>

        {expandedSection === 'links' && (
          <div id={`${sectionId}-links-content`} className="px-4 pb-4 border-t border-gray-700 bg-gray-900/30">
            {/* Distribution bars */}
      ...
      <Card className="overflow-hidden">
        <button
          ...
          aria-controls={`${sectionId}-flow-content`}
        >
          <div className="flex items-center gap-3">
            <span className="text-2xl" aria-hidden="true">🔀</span>
            <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-gray-500" aria-hidden="true">{expandedSection === 'flow' ? '▲' : '▼'}</span>
          </div>
        </button>

        {expandedSection === 'flow' && (
          <div id={`${sectionId}-flow-content`} className="px-4 pb-4 border-t border-gray-700 bg-gray-900/30">
            {/* Flow metrics */}
      ...
      <Card className="overflow-hidden">
        <button
          ...
          className="w-full p-4 text-left hover:bg-gray-700/30 transition-colors flex items-center justify-between"
          aria-expanded={expandedSection === 'ngrams'}
          aria-controls={`${sectionId}-ngrams-content`}
        >
          <div className="flex items-center gap-3">
            <span className="text-2xl" aria-hidden="true">📝</span>
            <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-gray-500" aria-hidden="true">{expandedSection === 'ngrams' ? '▲' : '▼'}</span>
          </div>
        </button>

        {expandedSection === 'ngrams' && (
          <div id={`${sectionId}-ngrams-content`} className="px-4 pb-4 border-t border-gray-700 bg-gray-900/30">
...
const IssueCard: React.FC<IssueCardProps> = ({ issue, fix, onApplyFix, isApplying }) => {
  ...
  return (
    <Card className="overflow-hidden">
      <button
        className="w-full p-4 text-left cursor-pointer hover:bg-gray-700/30 transition-colors"
      ...
      {expanded && (
        <div id={`${cardId}-details`} className="px-4 pb-4 border-t border-gray-700 bg-gray-900/30">
