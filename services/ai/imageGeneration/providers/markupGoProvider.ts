 ...
 * @deprecated Use markupGoProvider.generate() instead
 */
export async function generateHeroImage(
  ...
  brandKit: BrandKit,
  template: { id: string; markupGoTemplateId?: string },
  apiKey: string,
  timeoutMs: number = DEFAULT_TIMEOUT_MS
): Promise<string> {
  // Create a minimal placeholder and businessInfo for the provider
  const placeholder = {
    id: 'legacy',
    type: 'HERO' as const,
    position: 0,
    description: backgroundPrompt,
    altTextSuggestion: title,
  ...
  const businessInfo = {
    markupGoApiKey: apiKey,
    brandKit,
  } as BusinessInfo;

  const result = await markupGoProvider.generate(
    placeholder,
    {
      textOverlay: title,
      templateId: template.markupGoTemplateId || brandKit.markupGoDefaultTemplateId,
      altText: title,
    },
    businessInfo,
    { timeoutMs }
  ...
  if (!result.success || !result.imageUrl) {
    throw new Error(result.error || 'MarkupGo generation failed');
  }
